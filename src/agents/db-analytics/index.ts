...
import { getDBAnalyticsSystemPrompt } from './prompt.js';
import type { AgentInput, ParsedResponse } from '../../types/agent.js';

export class DBAnalyticsAgent extends BaseAgent {
  getSystemPrompt(): string {
    let prompt = getDBAnalyticsSystemPrompt(
  ...
  buildPrompt(input: AgentInput): string {
    const parts: string[] = [];
    if (input.query) parts.push(`## Analysis Request\n${input.query}`);
