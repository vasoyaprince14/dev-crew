...
import { getAuthArchitectSystemPrompt } from './prompt.js';
import type { AgentInput, ParsedResponse } from '../../types/agent.js';

export class AuthArchitectAgent extends BaseAgent {
  getSystemPrompt(): string {
    let prompt = getAuthArchitectSystemPrompt(this.projectInfo.framework || 'general');
  ...
  buildPrompt(input: AgentInput): string {
    const parts: string[] = [];
    if (input.query) parts.push(`## Auth Requirements\n${input.query}`);
