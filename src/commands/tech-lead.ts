...
export async function techLeadCommand(question?: string): Promise<void> {
  ...
  if (!agent) {
    logger.error('Failed to create tech-lead agent');
    process.exit(1);
  }

  spinner.start('Consulting tech lead...');
  try {
    ...
    spinner.fail('Tech lead consultation failed');
