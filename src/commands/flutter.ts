...
export async function flutterCommand(pathOrQuestion?: string): Promise<void> {
  ...
  const feedback = configManager.getFeedback('flutter');

  const registry = new AgentRegistry();
  const agent = registry.create('flutter', projectInfo, {}, feedback);
  if (!agent) {
    logger.error('Failed to create agent');
    process.exit(1);
  }

  spinner.start('Consulting Flutter expert...');
  try {
