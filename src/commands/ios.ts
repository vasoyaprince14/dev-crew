...
export async function iosCommand(pathOrQuestion?: string): Promise<void> {
  ...
  const feedback = configManager.getFeedback('ios');

  const registry = new AgentRegistry();
  const agent = registry.create('ios', projectInfo, {}, feedback);
  if (!agent) {
    logger.error('Failed to create agent');
    process.exit(1);
  }

  spinner.start('Consulting iOS expert...');
  try {
